...
struct Cli {
    ...
    #[arg(
        long = "full-section",
        help = "Return complete code sections (functions/classes) instead of just matching lines. Uses tree-sitter to identify semantic boundaries. Supported: Python, JavaScript, TypeScript, Haskell"
    )]
    full_section: bool,
...
async fn run_main() -> Result<()> {
    ...
    if cli.index {
        ...
        let stats =
            ck_index::smart_update_index_with_progress(&path, false, progress_callback, true)
                .await?;
