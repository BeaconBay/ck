...
use rmcp::model::{
    ...
    ListToolsResult, Meta, PaginatedRequestParams, ProgressNotificationParam, ProtocolVersion,
...
impl ServerHandler for CkMcpServer {
    fn get_info(&self) -> InitializeResult {
        ...
        InitializeResult::new(capabilities)
            ...
            .with_instructions(r#"CK is a semantic code search engine that helps you find code by meaning, not just text matching.
...
impl CkMcpServer {
    ...
    fn default_ckignore_route() -> ToolRoute<Self> {
        ...
        let tool = Tool::new(
            "default_ckignore",
            "Retrieve the default .ckignore content generated by ck",
            Arc::new(input_schema.as_object().unwrap().clone()),
        )
        .with_title("Default .ckignore");
    ...
    fn health_check_route() -> ToolRoute<Self> {
        ...
        ToolRoute::new_dyn(tool, |context: ToolCallContext<'_, CkMcpServer>| {
            Box::pin(async move {
                ...
                result.structured_content = Some(status_data);
                Ok(result)
            })
        })
    }
    ...
    fn regex_search_route() -> ToolRoute<Self> {
        ...
        let tool = Tool::new(
            "regex_search",
            "Search for code using regular expressions (grep-style)",
            Arc::new(input_schema.as_object().unwrap().clone()),
        )
        .with_title("Regex Search");
    ...
    fn hybrid_search_route() -> ToolRoute<Self> {
        ...
        let tool = Tool::new(
            "hybrid_search",
            "Hybrid search combining regex and semantic search with RRF ranking",
            Arc::new(input_schema.as_object().unwrap().clone()),
        )
        .with_title("Hybrid Search");
    ...
    fn index_status_route() -> ToolRoute<Self> {
        ...
        let tool = Tool::new(
            "index_status",
            "Get information about the index status for a directory",
            Arc::new(input_schema.as_object().unwrap().clone()),
        )
    ...
    fn reindex_route() -> ToolRoute<Self> {
        ...
        let tool = Tool::new(
            "reindex",
            "Force reindexing of a directory with progress tracking",
            Arc::new(input_schema.as_object().unwrap().clone()),
        )
        .with_title("Reindex Directory");
